...
namespace Library.Constant
{
  public class Constant
  {
    ...
    public const string DLTask_GetSumaryData = "SELECT (select count(ID) from task where StatusID = 1 and ProjectID = @ProjectID) AS Pending, (select count(ID) from task where StatusID = 2 and ProjectID = @ProjectID) AS Completed , (select count(ID) from task where StatusID = 3 and ProjectID = @ProjectID) AS CompletedLate;SELECT (select count(ID) from task where PriorityID = 1 and ProjectID = @ProjectID) AS Low, (select count(ID) from task where PriorityID = 2 and ProjectID = @ProjectID) AS Normal , (select count(ID) from task where PriorityID = 3 and ProjectID = @ProjectID) AS High;select count(ID) from task where ProjectID = @ProjectID;";
